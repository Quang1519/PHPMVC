...
class Admin extends Controller{
	...
	public function InsertData() {
		...
		switch ($_REQUEST['submit']) {
		 	...
		 	if(empty($tensp) || empty($gia) || empty($mau) || empty($soluong) || empty($hinh) || empty($mota) || $size == "0") {
			echo '<script language="javascript"> alert("Chưa điền đủ thông tin")</script>';
			}
			else {
				$sanpham = $this->model("SanPhamModel");
				// $result = $sanpham->InsertSP($tensp,$gia,$mau,$soluong,$size,$mota);
				$result = $sanpham->themxoasua(	"INSERT INTO sanpham(tensp,gia,mau,size,soluong,mota) VALUES ('$tensp','$gia','$mau','$size','$soluong','$mota')");
